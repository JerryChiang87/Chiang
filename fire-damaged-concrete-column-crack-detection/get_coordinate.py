...
for i in files:
    ...
    for j in range(len(contours)):
        ...
        ratio = w1/h1
        if cv2.contourArea(contours[j]) < 800 and cv2.contourArea(contours[j]) > 100 and (ratio>1.5 or ratio<0.5):     
            cnt = contours[j]
            for k in cnt:              
                for p in k:
                    x = p[0]
                    ...
                    #ax.contourf()
                    ax.scatter(x,y,z,c="red", marker = 2 , linewidths = 0.5)
                   
#        elif cv2.contourArea(contours[j])>50 and cv2.contourArea(contours[j])<100 and (ratio<1.5 or ratio>0.5):        
#            cnt = contours[j]
#            for k in cnt:              
#                for p in k:
#                    x = p[0]
#                    y = p[1]
#                ax.scatter(x,y,z,c="blue", marker = 1 , linewidths = 0.3)
            plt.pause(0.0000001)

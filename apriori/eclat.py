        ...
        if verbose:
            ...
            for j in range(min_combination, len(self.df_bin.columns) + 1):
                ...
                for i in tqdm(itertools.combinations(support_dict.keys(), r=j)):  # permutation
                    ...
                    dict_finally_index[separator.join(list(i))] = list(
                        self.df_bin.query('{}'.format(get_query)).loc[:, list(i)].index)
...
eclat_instance = ECLAT(
    data=pd.read_csv(r'C:\Users\Administrator\PycharmProjects\apriori\apriori\diana\eclat-diana.csv', header=None), verbose=False)

print(eclat_instance)  # generate a binary dataframe, that can be used for other analyzes.
# print(eclat_instance.uniq_)    #a list with all the names of the different items

get_ECLAT_indexes, get_ECLAT_supports = eclat_instance.fit(min_support=0.05,
                                                           min_combination=1,
                                                           max_combination=3,
                                                           separator='&',
                                                           verbose=False)

# print(sorted(get_ECLAT_indexes))
print(sorted(get_ECLAT_supports.items(), key=lambda x: x[1]))
# print(sorted(get_ECLAT_indexes.items(), key = lambda x: x[1]))

...
IMAGE_SIZE = [224, 224]
if version == 2:
    model = resnet_v2(input_shape=IMAGE_SIZE + [3], depth=depth)
else:
    model = resnet_v1(input_shape=IMAGE_SIZE + [3], depth=depth)
...
model_name = 'TRY4_%s_model.{epoch:03d}.h5' % model_type
...
training_set = ImageDataGenerator(rescale = 1./255).flow_from_directory('data_3categories/train',
                                                 target_size = (224, 224),
                                                 class_mode = 'categorical',
                                                 shuffle=True)

test_set = ImageDataGenerator(rescale = 1./255).flow_from_directory('data_3categories/test',
                                            target_size = (224, 224),
                                            class_mode = 'categorical',
                                            shuffle=False)
nb_train = 19100
...
if not data_augmentation:
    ...
    model.fit_generator(datagen.flow(training_set, batch_size=batch_size),
                        validation_data=(test_set),
...
#accuracy report
y_pred = model.predict_generator(test_set, nb_test//batch_size+1)
#print(y_pred)
y_pred = np.argmax(y_pred, axis=1)
#print(y_pred)
#fnames = test_set.filenames ## fnames is all the filenames/samples used in testing
#print(np.argmax(test_set.classes, axis=1))
